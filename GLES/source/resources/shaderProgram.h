...
public:
    ShaderProgram(const vulkanAPI::XContext_t *xContext = nullptr, vulkanAPI::CommandBufferManager *cbManager = nullptr);
    ~ShaderProgram();
    ...
    void                                                SetxContext(const vulkanAPI::XContext_t *xContext)  { FUN_ENTRY(GL_LOG_TRACE); mXContext = xContext; mPipelineCache->SetContext(mXContext); for (auto& gva : mGenericVertexAttributes) { gva.SetxContext(xContext); } }

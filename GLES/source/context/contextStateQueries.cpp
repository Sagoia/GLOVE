...
Context::GetIntegerv(GLenum pname, GLint* params)
{
    ...
    case GL_NUM_COMPRESSED_TEXTURE_FORMATS:     *params = static_cast<GLint>(mCompressedTextureFormats.size()); break;
...
Context::GetFloatv(GLenum pname, GLfloat* params)
{
    ...
    switch(pname) {
    case GL_ARRAY_BUFFER_BINDING:               *params = static_cast<GLfloat>(mStateManager.GetActiveObjectsState()->GetActiveBufferObject(GL_ARRAY_BUFFER) ? mResourceManager->GetBufferID(mStateManager.GetActiveObjectsState()->GetActiveBufferObject(GL_ARRAY_BUFFER)) : 0); break;
    ...
    case GL_ELEMENT_ARRAY_BUFFER_BINDING:       *params = static_cast<GLfloat>(mStateManager.GetActiveObjectsState()->GetActiveBufferObject(GL_ELEMENT_ARRAY_BUFFER) ? mResourceManager->GetBufferID(mStateManager.GetActiveObjectsState()->GetActiveBufferObject(GL_ELEMENT_ARRAY_BUFFER)) : 0); break;
